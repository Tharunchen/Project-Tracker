...
public class SampleUtilities extends Message {
	...
	public String insertdetails(SampleModel model) {
		...
		try {
			String selectQuery = "select * from registerdetails where username='" + model.getUsername()
					+ "' and deleteflag=0";
			...
			if (res.next()) {
				...
				String insertQuery = "INSERT INTO registerdetails (name, address, username, password, createdby, createddate) VALUES ('"
						+ model.getName() + "','" + model.getAddress() + "','" + model.getUsername() + "','"
						+ model.getPassword() + "','" + createdBy + "','" + ISTDate.getISTDateTime() + "')";

				int result = DatabaseOperations.insertAndGetId(insertQuery);

				String insertQuery1 = "INSERT INTO role_assigned (username) VALUES ('" + model.getUsername() + "')";
	...
	public String updateSkill(SampleModel model) {
		...
		String updateQuery = "UPDATE registerdetails SET " + "name='" + model.getName() + "', " + "address='"
				+ model.getAddress() + "', " + "username='" + model.getUsername() + "' " + "password='"
				+ model.getPassword() + "' " + "WHERE slno='" + model.getSlno() + "'";
	...
	public List<RolesDetails> getdropdownusername() {
		...
		try {
			ResultSet res = DatabaseOperations
					.select("SELECT username FROM role_assigned where deleteflag=0 group by username");
